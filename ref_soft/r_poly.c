	...
	for (i=0 ; i<nump ; i++, instep += sizeof (vec5_t) / sizeof (float))
	{
		...
		if (dists[i] == 0 || dists[i+1] == 0)
			continue;
...
#if defined (__APPLE__) || defined (MACOSX)
                // <AWE> an "equal" comparision nested between two "greater than comprision" will
                //	 cause the gcc3 to bail out because of a bug, our fix:
                cmp1 = dists[i] > 0;
                cmp2 = dists[i+1] > 0;
                ...
                // shoudl save some cycles compared to:
                //if ((dists[i] > 0 && dists[i] > 0) || (dists[i] <= 0 && dists[i] <= 0))
                //    continue;
#else
		if ( (dists[i] > 0) == (dists[i+1] > 0) )
			continue;
